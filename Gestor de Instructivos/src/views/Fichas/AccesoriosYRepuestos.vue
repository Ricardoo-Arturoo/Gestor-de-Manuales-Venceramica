...
const cargarAccesoriosYRepuestos = async () => {
  try {
    // CORRECCIÓN: Se agregan %20 para respetar los espacios de "Accesorios y Repuestos" en la URL
    ...
    console.log("Respuesta de la API corregida:", datosObtenidos) 

    // Los filtros ahora sí encontrarán los datos
    accesorios.value = datosObtenidos.filter(h => h.tipo === 'Accesorios' && h.clasificacion === 'Ficha')
    repuestos.value = datosObtenidos.filter(h => h.tipo === 'Repuestos' && h.clasificacion === 'Ficha')
...
const procesarDescarga = (archivoPdf) => {
  if (!archivoPdf) return alert("Este producto aún no tiene una ficha técnica asignada.")
  window.open(`https://api.instructivos.venceramica.com/api/descargar/${archivoPdf}`, '_blank')
}

const eliminarProducto = async (id) => {
  if(confirm('¿Estás seguro de que deseas eliminar este producto y su ficha técnica?')) {
    try {
...
<template>
  <div class="flex h-screen w-full bg-gray-50 font-sans overflow-hidden">
    ...
    <main class="flex-1 w-full h-full overflow-y-auto pt-20 px-6 pb-12 md:p-12 relative">
      <HeaderVista titulo="Fichas Técnicas de Accesorios y Repuestos" descripcion="Seleccione el modelo de accesorio o repuesto para descargar su ficha técnica." />
