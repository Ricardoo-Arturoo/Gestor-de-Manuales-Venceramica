...
const cargarGriferias = async () => {
  try {
    const respuesta = await fetch('http://localhost:3000/api/productos/Griferias')
    if (!respuesta.ok) throw new Error('Error al conectar con el servidor')
    
    const datosObtenidos = await respuesta.json()

    // Filtrado según la nueva categorización
    bidet.value = datosObtenidos.filter(g => g.tipo === 'Bidet')
    ducha.value = datosObtenidos.filter(g => g.tipo === 'Ducha')
    lavamanos.value = datosObtenidos.filter(g => g.tipo === 'Lavamanos')
    cocinas.value = datosObtenidos.filter(g => g.tipo === 'Cocinas')
    fluxometros.value = datosObtenidos.filter(g => g.tipo === 'Fluxómetros')
    llavesTemporizadas.value = datosObtenidos.filter(g => g.tipo === 'Llaves Temporizadas')
