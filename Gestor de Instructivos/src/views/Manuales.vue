...
const categorias = ref([
  ...
  { id: 6, nombre: 'Herrajes', imagen: '/img/herrajes.png' }, 
  { id: 7, nombre: 'Griferias', imagen: null },
...
const seleccionarCategoria = (nombre) => {
  ...
  if (nombre === 'Inodoros') {
    ...
    // Ejemplo: "Bidet" se convierte en "bidet"
  ...
  <div class="flex h-screen w-full bg-gray-50 font-sans overflow-hidden">
    ...
    <main class="flex-1 w-full h-full overflow-y-auto pt-20 px-6 pb-12 md:p-12 relative">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 animacion-entrada-retraso">
        ...
        <TarjetaCategoria v-for="categoria in categorias" :key="categoria.id" :titulo="categoria.nombre"
          :imagen="categoria.imagen" subtitulo="Ver manuales" @click="seleccionarCategoria(categoria.nombre)" />
      </div>
    </main>
  </div>
</template>
