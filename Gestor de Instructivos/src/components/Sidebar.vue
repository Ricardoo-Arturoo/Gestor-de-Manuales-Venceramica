  ...
  >
    ...
    <div class="bg-[#CE1126] flex items-center justify-center h-20 md:h-24 shadow-md px-4 relative flex-shrink-0">
      <img 
        ...
        class="h-full w-auto max-h-16 object-contain scale-120 drop-shadow-md mb-3 relative z-10 mt-2" 
      />
      
      <!-- Botón X (Adentro de la cabecera del sidebar, alineado a la derecha) -->
      <button 
        @click="isOpen = false"
        class="md:hidden absolute right-3 top-1/2 -translate-y-1/2 p-2 bg-black/8 hover:bg-black/20 text-white rounded-md transition-colors z-20"
      >
        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <nav class="flex-1 px-4 py-8 space-y-2 overflow-y-auto">
      ...
      <router-link 
        to="/inicio" 
        @click="isOpen = false"
        class="group flex items-center gap-3 px-4 py-3 rounded-xl text-gray-500 hover:text-[#AB1A2D] transition-all duration-300 font-semibold text-xl"
        active-class="!text-[#AB1A2D] shadow-sm"
      >
