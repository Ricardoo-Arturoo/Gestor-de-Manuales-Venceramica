<template>
  ...
  <div
    ...
    <div @click="manejarClicPrincipal"
      ...
      <div
        ...
        <!-- Si no hay imagen, muestra el slot o el icono por defecto -->
      ...
      <div class="flex-1">
        <h3 class="text-xl font-bold text-gray-800 transition-colors" :class="{ 'text-[#AB1A2D]': menuAbierto }">
          {{ titulo }}
        </h3>
      ...
      <button v-if="tieneSubcategorias" @click.stop="toggleMenu"
        ...
        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 transition-transform duration-500"
          :class="{ 'rotate-45 text-[#CE1126]': menuAbierto }" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div
      ...
      <div class="flex-1 overflow-y-auto space-y-1 pr-1 custom-scrollbar">
        <button v-for="sub in subcategorias" :key="sub.id" @click.stop="manejarClicSubcategoria(sub.id)"
...
const props = defineProps({
  ...
  imagen: {
    type: [String, Object], // Permite recibir tanto la ruta en texto como la variable importada de Vite
    default: null
  },
  subcategorias: {
    type: Array,
